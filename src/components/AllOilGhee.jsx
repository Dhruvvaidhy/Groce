...
const AllOilGhee = () => {
  ...
  useEffect(() => {
    const fetchOilGheeProducts = async () => {
      try {
        const response = await axios.get("http://localhost:5000/oilGhee");
        setProducts(response.data);
      } catch (error) {
        console.error("Error fetching oil & ghee products:", error);
      }
    };

    fetchOilGheeProducts();
  }, []);
  ...
  return (
    <div className="AllOilGhee mx-4 mt-10">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold">All Oil & Ghee Products</h2>
        <button
